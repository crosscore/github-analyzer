...
class GitHubRepoAnalyzer:
    ...
    def get_file_content(self, file_url: str) -> str:
        if file_url in self.content_cache:
            return self.content_cache[file_url]

        response = requests.get(file_url, headers=self.headers)
        response.raise_for_status()
        content = base64.b64decode(response.json()['content']).decode('utf-8')
        self.content_cache[file_url] = content
        return content
        ...
        def process_path(path: str, prefix: str = ''):
            ...
            for item in sorted(items, key=lambda x: (x['type'] != 'dir', x['name'])):
                ...
                if item['type'] == 'dir':
                    ...
                    structure.append(structure_path)
                    try:
                        contents[full_path] = self.get_file_content(item['download_url'])
...
def main():
    ...
    default_filename = f"{analyzer.repo}.txt"
